...
def main(argv):
    ...
    '''
    try:
        opts, args = getopt.getopt(argv,"hasvfn:t::",["auto","help","silent","verbose","fiteclipse","threshold="])
    ...
    pcloud_files = getFiles(os.getcwd(),'.csv')
    ...
    for pcf in pcloud_files:
        if not 'intermidiate' in pcf and not 'result' in pcf:
            ...
            # apply filter
            pc_filtered = pcfilter.ClusterFilterXY(PointCloud_xy,50,50,50)
            ...
            for idx in range(0, CAMParams.__len__()):
                ...
                plt.plot([0,axis_pt_x], [0, axis_pt_y],label=label_str, lw=0.2, alpha=0.2)
